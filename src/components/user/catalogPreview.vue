<template>
    <div class="catalogPreview">
        <div class="article_none_pre" v-show="checked.length?!original.length:!alldata.length">
            <img :src="none">
            <div>空空如也</div>
            <router-link to="/md/mavon" style="text-decoration: none;"><span>写博客</span></router-link>
        </div>

        <div class="banner_pre" v-show="checked.length?original.length:alldata.length">
        ...
        <div class="catalog_pre" v-for="(item,index) in checked?original:alldata" :key="index">
            <div class="title">
...
<style lang="less">
    ...
    .banner_pre{display: flex;height: 4rem;align-items: center;border-bottom: 1px solid #000;padding: .5rem 1rem .5rem 2rem;
    ...
    .catalog_pre{display: flex;flex-direction:column;padding: 1rem 2rem;border-bottom: 1px dashed rgba(0,0,0,.2)!important;transition: background-color .25s;
    ...
    .article_none_pre{display: flex;width: 100%;align-items: center;justify-content: center;flex-direction: column;padding-top: 10rem;
